...
import (
	...
	"github.com/TameshkCloud/todoberry-server/services/redis_client"
)
...
func SayHello(ctx *gin.Context) {
	ctx.JSON(200, gin.H{
		"status": true,
		"message":   "Hello! My name is TodoBerry",
	})
}


// Response struct
type GetRedisConfigurationResponse struct {
	RedisIP string `json:"redis_ip"`
	RedisCluster bool `json:"cluster"`
	Message string `json:"message"`
}

func CheckRedis(ctx *gin.Context) {
	var response GetRedisConfigurationResponse
	// An example for howto load configuration details into controller
	// and add it to a response struct
	redisConfig := config.RedisConfig{}
	config.LoadConfig("redis",&redisConfig)
	
	response.RedisIP = redisConfig.DSN[0]
	response.RedisCluster = redisConfig.Cluster

	// redis test
	redisClient := redis_client.New(redis_client.DEFAULT_REDIS)
	ctx.Set(redis_client.DEFAULT_REDIS, redisClient)

	response.Message = "key has been set"
	_,err := redisClient.Set("todoberry:test", "1", 30 * time.Minute).Result()
	if err != nil {
		response.Message = "error while setting key in redis " + err.Error()	
	}
